...
# drop RST
sudo ip netns exec host1 iptables -A OUTPUT -p tcp --tcp-flags RST RST -j DROP
sudo ip netns exec host2 iptables -A OUTPUT -p tcp --tcp-flags RST RST -j DROP

# turn off checksum offloading
sudo ip netns exec host2 ethtool -K host2-veth1 tx off
sudo ip netns exec host1 ethtool -K host1-veth1 tx off
